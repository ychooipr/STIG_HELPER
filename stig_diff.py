...
def _open_local_path(path_value):
    ...
    if sys.platform == "win32":
        os.startfile(target)  # nosemgrep: validated local path, no shell
    elif sys.platform == "darwin":
        subprocess.Popen(["open", target])  # nosemgrep: constant command, validated local path
    else:
        subprocess.Popen(["xdg-open", target])  # nosemgrep: constant command, validated local path
